...
def test_full_integration_workflow() -> None:
    ...
    response = client.post("/sensors/", 
                           json={"id": sensor_id, "type": "temp", "name": "Prueba"})
    assert response.status_code == 201
    
    # Conflicto: Crear mismo sensor (409)
    response_conflict = client.post("/sensors/", 
                                    json={"id": sensor_id, "type": "temp", 
                                          "name": "Prueba"})
    ...
    response = client.post(f"/sensors/{sensor_id}/readings", 
...
def test_error_handlers() -> None:
    ...
    assert client.post("/sensors/FAKE/readings", 
                       json={"value": 10, "unit": "C"}).status_code == 404
...
def test_physics_validation() -> None:
    ...
    sensor_id = get_unique_id()
    client.post("/sensors/", json={"id": sensor_id, 
                                   "type": "temp", "name": "Test Físico"})
