...
def test_full_system_integration(tmp_path: Path) -> None:
    ...
    total_readings = run_simulation(num_sensors=10, cycles=60, log_path=str(log_file))

    # Assert 1: Se debieron procesar exactamente 600 lecturas
    assert total_readings == 600

    # Assert 2: El archivo de log debe existir y contener alertas
    # generadas por el simulador
